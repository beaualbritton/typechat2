    ...
    return(
      <main className="flex flex-col h-screen w-full items-center justify-center">
        <div className="p-4 bg-elevated w-2xl h-96 rounded border-main border-2 flex flex-col items-center justify-center">
          <h1 className="text-5xl text-green font-bold p-2">Enter Username</h1>
          <form className="w-2xl flex flex-col items-center justify-center p-4" onSubmit={(e) => {e.preventDefault(); setConnection(true);}}>
            <input className="bg-surface text-xl border border-main text-yellow rounded w-full p-4" value={username} onChange={(e) => setUsername(e.target.value)}/>
  ...
  return (
    <main className="flex flex-col h-screen w-full items-center justify-center">
      ...
      <form className="w-2xl flex items-center bg-surface border border-main rounded p-4" onSubmit={submit}>
        <span className="text-green text-xl">&gt;&nbsp;</span> <input className="bg-surface text-xl text-yellow flex-1 outline-none" value={input} onChange={(e) => setInput(e.target.value)}/>
